# [PROJECT NAME] — Merge Plan Session History

Older session-journal blocks from `MERGE_PLAN.md`, retained here for narrative continuity. The **5 most recent** blocks stay in the main file's `## Session journal (recent)` section; everything older lives here.

The main file keeps its recent session-journal blocks in a `## Session journal (recent)` section (blank-line-separated, newest-first); its `Last updated:` line is just a one-line pointer. As blocks accumulate, the main file's session-load cost grows; rolling older blocks into this file keeps the main file scannable without losing the trail.

---

## When to roll an entry here

At session close, if the `## Session journal (recent)` section in `MERGE_PLAN.md` is carrying more than 5 blocks, move the oldest one (or two) into this file.

Order in this file: **newest first**, same as the section order in the main file. The most recent block rolled out of the main file goes at the top of this file's session list.

Don't summarize. Move the entries verbatim. The whole point of the history file is that it preserves the original narrative without compression — `MERGE_PLAN.md` is the working file, this is the archive.

---

## Session entries (newest first)

PRIOR — YYYY-MM-DD [session label]. [Verbatim entry rolled out of `MERGE_PLAN.md`.]

PRIOR — YYYY-MM-DD [session label]. [Verbatim entry rolled out of `MERGE_PLAN.md`.]

[...]

---

*Part of [4SYNC-CMS](https://github.com/SandmanCircles/4SYNC-CMS). MIT license.*
